...
public class SWO_CyclicBehaviour_SingleAgent extends CyclicBehaviour {
    ...
    public SWO_CyclicBehaviour_SingleAgent(int totalAgents, GRBModel model, Parameters parameters, ADMMDataModel admmDataModel, Set<Electrolyzer> electrolyzers, Set<Period> periods, double rho, int iteration, int maxIterations) {
    ...
    public void action() {
        ...
        // SINGLE AGENT MODE: Execute complete iteration sequence
        if (totalNumberADMMAgents == 1) {
            startCompleteIterationSequence();
            // Block this behaviour until the sequence is complete
            block();
        } else {
            // MULTI-AGENT MODE: Original message receiving logic
    ...
    /**
     * Starts the complete iteration sequence for single agent mode
     */
    private void startCompleteIterationSequence() {
        // Create sequential behaviour for the complete iteration
        SequentialBehaviour iterationSequence = new SequentialBehaviour();
        
        // Add all behaviours in the correct order
        iterationSequence.addSubBehaviour(new SWO_XUpdateBehaviour(model, parameters, parameters.getElectrolyzers(), 
            dataModel.getAssignedPeriods(), swoIterationCount, dataModel, rho, currentStartPeriod));
        
        iterationSequence.addSubBehaviour(new SWO_YUpdateBehaviour(model, parameters, electrolyzers, periods, 
            swoIterationCount, dataModel, rho, e -> electrolyzers.contains(e), currentStartPeriod));
        
        iterationSequence.addSubBehaviour(new SWO_SUpdateBehaviour(model, parameters, electrolyzers, periods, 
            swoIterationCount, dataModel, rho, e -> electrolyzers.contains(e)));
        
        iterationSequence.addSubBehaviour(new SWO_DualUpdateBehaviour(parameters, electrolyzers, periods, 
            swoIterationCount, dataModel, rho, e -> electrolyzers.contains(e)));
        
        // Add a custom behaviour for convergence check and iteration increment
        iterationSequence.addSubBehaviour(new jade.core.behaviours.OneShotBehaviour() {
            @Override
            public void action() {
                // Calculate residuals
                ...
                if (checkFeasibility() && swoIterationCount > 0) {
                    ...
                    System.out.println("Convergence reached after iteration " + swoIterationCount + ". Terminating optimization.");
                    saveSWOResultsAndTerminate();
                } else {
                    // Increment iteration counter for next iteration
                    swoIterationCount++;
                    System.out.println("No convergence after iteration " + (swoIterationCount - 1) + ". Starting iteration " + swoIterationCount);
                    
                    // Start the next iteration automatically
                    if (swoIterationCount < maxIterations) {
                        startCompleteIterationSequence();
                    }
                }
        ...
        // Add the complete sequence to the agent
        myAgent.addBehaviour(iterationSequence);
    }
    ...
    private void handleSWO_XUpdateMessage(String content) {
        ...
        if (parts.length >= 2) {
            ...
            if (iteration != swoIterationCount) {
                System.err.println("Incorrect SWO iteration!!");
            }
            ...
            for (int i = 2; i < parts.length; i++) {
                ...
                if (resultParts.length == 4) {
                    ...
                    output.append("Agent: ").append(myAgent.getLocalName())
                          ...
                          .append(", Period: ").append(periodIndex + 1)
    ...
    private void saveSWOResultsAndTerminate() {
        ...
        try {
            ...
            if (dataModel.getXSWOValuesForIteration(swoIterationCount + 1) != null) {  // +1 für aktuelle Werte
                dataModel.exportFinalIterationResultsToExcel(swoIterationCount + 1, parameters.getElectrolyzers(), parameters.getPeriods(), parameters, excelFilePathFinalResults);  // +1 für aktuelle Werte
                System.out.println("Final results successfully exported to: " + excelFilePathFinalResults);
            } else {
                System.out.println("Warning: No data available for final iteration " + (swoIterationCount + 1) + ". Skipping final results export.");
            }
            
            // Export all variables per electrolyzer to separate Excel files
            if (myAgent.getLocalName().equals("ADMM3")) {
                try {
                    String baseFilePath = desktopPath + "/" + saveDetails + "_ADMM_Variables_Iteration_" + (swoIterationCount + 1);  // +1 für aktuelle Werte
                    dataModel.exportAllVariablesPerElectrolyzerToExcel(
                        swoIterationCount + 1,  // +1 für aktuelle Werte
    ...
    private void calculateBoundaryResiduals() {
        ...
        for (Electrolyzer e : dataModel.getAllElectrolyzers()) {
            ...
            for (Period t : periods) {
                ...
                double xValue = dataModel.getXSWOValueForAgentPeriod(swoIterationCount + 1, agentIndex, periodIndex);  // +1 für aktuelle Werte
                boolean isProducing = dataModel.getYSWOValuesForAgent(swoIterationCount + 1, agentIndex)[periodIndex][State.PRODUCTION.ordinal()];  // +1 für aktuelle Werte
    ...
    public static boolean checkFeasibilityAndCalculateObjective(Set<Period> currentPeriods,
	    ...
	    double[][] xValues = dataExchange.getXSWOValuesForIteration(admmIter + 1);  // +1 für aktuelle Werte
	    boolean[][][] yValues = dataExchange.getYSWOValuesForIteration(admmIter + 1);  // +1 für aktuelle Werte
