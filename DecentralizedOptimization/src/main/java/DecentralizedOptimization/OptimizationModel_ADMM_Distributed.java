...
public class OptimizationModel_ADMM_Distributed {
	/**
	 * Global Variables
	 */
	...
	public static void main(String[] args) {
		...
		try {
			...
			dataExchange.initializeDualVariablesForFirstIteration();
			...
			for (int admmIter = 0; admmIter < maxADMMIterations - 1; admmIter++) {
				...
				for (Agent agent : agents) {
					try (IloCplex cplex = new IloCplex()) {
						...
						agent.optimizeX(cplex, params, filteredPeriods, agents, admmIter, dataExchange, rho);
				...
				for (Agent agent : agents) {
					try (IloCplex cplex = new IloCplex()) {
						configureCplex(cplex);
						agent.optimizeY(cplex, params, currentPeriods, admmIter, dataExchange, rho);
					} catch (IloException e) {
						e.printStackTrace();
					}
				}
				...
				for (Agent agent : agents) {
					try (IloCplex cplex = new IloCplex()) {
						configureCplex(cplex);
						agent.optimizeS(cplex, params, currentPeriods, admmIter, dataExchange, rho);
					} catch (IloException e) {
						e.printStackTrace();
					}
				}
				...
				for (Agent agent : agents) {
					try (IloCplex cplex = new IloCplex()) {
						configureCplex(cplex);
						agent.updateU(cplex, params, currentPeriods, admmIter, dataExchange, rho);
					} catch (IloException e) {
						e.printStackTrace();
					}
				}
				...
				if (admmIter > 0) {
					double improvement = Math
							.abs((previousObjectiveValue - currentObjectiveValue) / previousObjectiveValue);
					System.out.println("Verbesserung von Iteration " + admmIter + ": " + (improvement * 100) + "%"
							+ " previousObjective: " + previousObjectiveValue + " currentObjective: "
							+ currentObjectiveValue);
					boolean feasible = dataExchange.getFeasibilityForIteration(admmIter+1);
			...
			dataExchange.writeXYValuesToExcel("XYValues.xlsx");
	...
	public static double checkFeasibilityAndCalculateObjective(Set<Agent> agents, Set<Period> currentPeriods,
		...
		double tolerancePercentage = 0.02; // 0,5% Toleranz erlaubt
		double zeroTolerance = 0.1; // Feste Toleranz, wenn Grenzwert Null ist
		...
		for (Agent agent : agents) {
			...
			for (Period period : currentPeriods) {
				...
				if (Math.abs(ySum - 1) > ySumTolerance) {
					System.out.println("Nebenbedingungsverletzung: Summe der y-Variablen (" + ySum
							+ ") ungleich 1 für Agent " + agent.getId() + " in Periode " + period.getT());
					feasible = false;
				}

				// Nebenbedingung 2: x ≥ 0
				double lowerBound = 0.0;
				double lowerTolerance = (lowerBound == 0.0) ? zeroTolerance
						: tolerancePercentage * Math.abs(lowerBound);
				if (x < lowerBound - lowerTolerance) {
					System.out.println("Nebenbedingungsverletzung: x (" + x + ") < 0 (mit Toleranz " + lowerTolerance
							+ ") für Agent " + agent.getId() + " in Periode " + period.getT());
					feasible = false;
				}
				...
				double upperTolerance = (xUpperBound == 0.0) ? zeroTolerance
						: tolerancePercentage * Math.abs(xUpperBound);

				if (x > xUpperBound + upperTolerance) {
					System.out.println("Nebenbedingungsverletzung: x (" + x + ") überschreitet obere Grenze ("
							+ xUpperBound + ") plus Toleranz (" + upperTolerance + ") für Agent " + agent.getId()
							+ " in Periode " + period.getT());
					feasible = false;
				}
				...
				double lowerToleranceBound = (xLowerBound == 0.0) ? zeroTolerance
						: tolerancePercentage * Math.abs(xLowerBound);

				if (x < xLowerBound - lowerToleranceBound) {
					System.out.println("Nebenbedingungsverletzung: x (" + x + ") unterschreitet untere Grenze ("
							+ xLowerBound + ") minus Toleranz (" + lowerToleranceBound + ") für Agent " + agent.getId()
							+ " in Periode " + period.getT());
					feasible = false;
				}
				...
				if (period.getT() > 1) {
					...
					if (lhs_STARTING && !rhs_STARTING) {
						System.out.println("Nebenbedingungsverletzung: Ungültiger Übergang zu STARTING für Agent "
								+ agent.getId() + " in Periode " + period.getT());
						feasible = false;
					}

					// Nebenbedingung: y_t,PRODUCTION ≤ y_{t-1,PRODUCTION} + y_{t-1,STANDBY} +
					// y_{t-startupDuration,STARTING}
					...
					if (lhs_PRODUCTION && !rhs_PRODUCTION) {
						System.out.println("Nebenbedingungsverletzung: Ungültiger Übergang zu PRODUCTION für Agent "
								+ agent.getId() + " in Periode " + period.getT());
						feasible = false;
					}
					...
					if (lhs_STANDBY && !rhs_STANDBY) {
						System.out.println("Nebenbedingungsverletzung: Ungültiger Übergang zu STANDBY für Agent "
								+ agent.getId() + " in Periode " + period.getT());
						feasible = false;
					}

					// Nebenbedingung: y_t,IDLE ≤ y_{t-1,IDLE} + y_{t-1,PRODUCTION} +
					// y_{t-1,STANDBY}
					boolean lhs_IDLE = y[State.IDLE.ordinal()];
					boolean rhs_IDLE = yPrev[State.IDLE.ordinal()] || yPrev[State.PRODUCTION.ordinal()]
							|| yPrev[State.STANDBY.ordinal()];
					if (lhs_IDLE && !rhs_IDLE) {
						System.out.println("Nebenbedingungsverletzung: Ungültiger Übergang zu IDLE für Agent "
								+ agent.getId() + " in Periode " + period.getT());
						feasible = false;
					}
					...
					// Toleranz für die Rampenratenüberprüfung definieren
					double rampTolerance = 0.001; // Beispielwert für eine feste Toleranz

					// Überprüfung der Rampenratenbedingungen
					if (period.getT() > 1) {
						double previousXValue = xValues[agentIndex][prevPeriodIndex];
						boolean[] prevY = yValues[agentIndex][prevPeriodIndex];

						// Rampenrate-Bedingung 1: x_{a,t} - x_{a,t-1} ≤ RampRateMax * Δ_int *
						// y_{a,t,production}
						double diff1 = Math.abs(currentXValue - previousXValue);
						double rampRate = 0.6; // Beispielwert für die maximale Rampenrate

						// Berechnung der tolerierten Rampenrate
						double rampUpConstraint1 = rampRate;
						double upperRampTolerance = (rampUpConstraint1 == 0.0) ? rampTolerance
								: rampTolerance * Math.abs(rampUpConstraint1);
		...
		for (Period period : currentPeriods) {
			...
			for (Agent agent : agents) {
				...
				// Berechnung der Produktion: intercept wird nur verwendet, wenn
				// isProductionActive == 1
	...
	private static Parameters loadParameters(Workbook workbook) {
		...
		// Rückgabe der Parameter einschließlich des periodenspezifischen Demand und der
		// Haltedauern
	...
	public static void exportFinalIterationResultsToExcel(DataExchange dataExchange, int finalIteration,
			Set<Agent> agents, Set<Period> periods, Parameters params, String filePath) throws IOException {
		...
		for (Period period : periods) {
			...
			for (Agent agent : agents) {
				...
				row.createCell(4).setCellValue(hydrogenProductionValues[agentIndex][periodIndex]); // Hydrogen
																									// Production
	...
	private static void configureCplex(IloCplex cplex) throws IloException {
		...
		cplex.setParam(IloCplex.Param.MIP.Display, 0); // Disable detailed MIP output
	}
}
