...
public class DataExchange {
    ...
    public void initializeIteration(int iteration, int numPer, int numAgents) {
        ...
        this.U.put(iteration, new double[numAgents][numPer][4]);
        this.hydrogenProduction.put(iteration, new double[numAgents][numPer]);
    }
    
    
    public void initializeDualVariablesForFirstIteration() {
        int firstIteration = 0; // Annahme, dass mit der ersten Iteration begonnen wird

        // Anzahl der Agenten und Perioden aus den bestehenden Daten ableiten
        int numAgents = X.get(firstIteration).length;
        int numPeriods = X.get(firstIteration)[0].length;

        // Initialisiere U-Werte für die erste Iteration mit 4 Residualwerten (einschließlich Rampen)
        double[][][] uValues = new double[numAgents][numPeriods][4];
        for (int i = 0; i < numAgents; i++) {
            for (int j = 0; j < numPeriods; j++) {
                Arrays.fill(uValues[i][j], 0.0); // Setze alle U-Werte auf 0.0
            }
        }

        // Speichere die initialisierten U-Werte in der Map für die erste Iteration
        saveUValuesForIteration(firstIteration, uValues);
    }
 ...
 // Methode zum Speichern der X-Werte für eine bestimmte Iteration und einen Agenten
    ...
    public void writeValuesToExcel_sorted(String filePath) {
        ...
        for (int agentIndex = 0; agentIndex < X.get(0).length; agentIndex++) {
            for (int periodIndex = 0; periodIndex < X.get(0)[agentIndex].length; periodIndex++) {
                ...
                headerRow.createCell(headerColumn++).setCellValue(baseHeader + "U4");  // Neue Spalte für U4 (Rampenraten)
        ...
        for (Integer iteration : X.keySet()) {
            ...
            for (int agentIndex = 0; agentIndex < X.get(iteration).length; agentIndex++) {
                for (int periodIndex = 0; periodIndex < X.get(iteration)[agentIndex].length; periodIndex++) {
                    ...
                    // U-Werte
                    for (int i = 0; i < 4; i++) { // Jetzt 4 Werte für U (einschließlich U4 für Rampenraten)
    ...
    public void writeXYValuesToExcel(String filePath) {
        XSSFWorkbook workbook = new XSSFWorkbook();  // Neues Workbook erstellen
        XSSFSheet sheet = workbook.createSheet("XY Values");  // Neues Sheet erstellen
        ...
        for (Integer iteration : X.keySet()) {
            ...
            // Iteriere über die Agenten und Perioden, um die X und Y Werte zu schreiben
            for (int agentIndex = 0; agentIndex < X.get(iteration).length; agentIndex++) {
                for (int periodIndex = 0; periodIndex < X.get(iteration)[agentIndex].length; periodIndex++) {
                    // X-Wert
                    Cell cellX = row.createCell(colNum++);
 ...
 // Methode zum Abrufen der S-Werte eines bestimmten Agenten für eine bestimmte Iteration
    ...
    // Anpassung der Methode zum Speichern der U-Werte für einen bestimmten Agenten und eine bestimmte Iteration
    ...
    // Anpassung der Methode zum Speichern eines U-Wertes für eine bestimmte Iteration, Agenten und Periode
    public void saveUValueForAgentPeriod(int iteration, int agentIndex, int periodIndex, int residualIndex, double uValue) {
        ...
        // Überprüfe, ob der agentIndex, periodIndex und residualIndex gültig sind
