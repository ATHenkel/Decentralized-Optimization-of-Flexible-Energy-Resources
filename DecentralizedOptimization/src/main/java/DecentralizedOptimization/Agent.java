...
public class Agent {
	...
	public void optimizeX(IloCplex cplex, Parameters params, Set<Period> periods, Set<Agent> agents, int iteration,
			DataExchange dataExchange, double rho)
			throws IloException, IOException {
		xUpdate xUpdate = new xUpdate();
		xUpdate.optimizeX(cplex, params, agents, periods, iteration, dataExchange, rho);
	}

	// Agent-specific y-update method
	public void optimizeY(IloCplex cplex, Parameters params, Set<Period> periods, int iteration,
			DataExchange dataExchange, double rho)
			throws IloException {
		yUpdate yUpdate = new yUpdate();
		yUpdate.optimizeY(cplex, params, this, periods, iteration, dataExchange, rho);
	}

	// Agent-specific s-update method
	public void optimizeS(IloCplex cplex, Parameters params, Set<Period> periods, int iteration,
			DataExchange dataExchange, double rho)
			throws IloException {
		sUpdate sUpdate = new sUpdate();
		sUpdate.optimizeS(cplex, params, this, periods, iteration, dataExchange, rho);
	}

	// Agent-specific dual update method
	public void updateU(IloCplex cplex, Parameters params, Set<Period> periods, int iteration,
			DataExchange dataExchange, double rho)
			throws IloException {
		dualUpdate dualUpdate = new dualUpdate();
		dualUpdate.optimizeU(cplex, params, this, periods, iteration, dataExchange, rho);
	}
